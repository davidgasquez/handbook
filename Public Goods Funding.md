...
- **Anti-Goodhart Resilience**. Any metric used for decisions will be gamed. Mechanisms should assume this, incorporate **[[Feedback Loops|feedback loops]] and error analysis**, and make it easy to update or combine metrics and evaluators when they drift from what really matters.
...
- **Composable Data and Evaluation Layers**. Separate **data collection** (attestations about work, usage, dependencies, etc.) from **judgment** (how that data is weighted). Multiple evaluation "lenses" (models, juries, [[Dashboards|dashboards]]) should be able to operate on the same shared data structures (graphs, weight vectors).
