...
- The data team needs to be focused on delivering insights and supporting decisions. The outcomes of the data team are _decisions_ and a _shared context across the [[Organizations|organization]]_ that makes [[Coordination|coordination]] easier.
  ...
  - Before starting data work, name the decision owner, the decision, the intervention for a high/low/uncertain result, and the downstream outcome. [A model with no plan to intervene based on its output is not a success](https://ludic.mataroa.blog/blog/most-data-work-seems-fundamentally-worthless/). [Data doesn't make decisions, people do](https://twitter.com/teej_m/status/1765475939084029956).
- [Data's impact is tough to measure — it doesn't always translate to value](https://dfrieds.com/articles/data-science-reality-vs-expectations.html)
  ...
  - Measure the full value chain: artifact used → decision changed → intervention made → outcome observed. Usage is not proof of value, but [zero usage is strong evidence of waste](https://ludic.mataroa.blog/blog/most-data-work-seems-fundamentally-worthless/).
...
- [Data teams should be a part of the business conversations from the beginning](https://cultivating-algos.stitchfix.com/). Get the data team involved early, have open discussions with them about the existing work, and how to prioritize new work against the existing backlog. Don't accept new work without addressing the existing bottlenecks, and don't accept new work without requirements. **Organizational [[politics]] matter way more than any data methods or technical knowledge**. The hard bit about becoming data driven in business isn't the technical bits. It's the political bits.
  - Talking about data is often [[Signaling|signalling]]. Compare ["data-driven" language with revealed behavior](https://ludic.mataroa.blog/blog/most-data-work-seems-fundamentally-worthless/). Budgets, workflow changes, willingness to fix foundations, and whether anyone acts on the output.
...
- The purpose of becoming data driven is to build a causal model of the business in your head. The purpose of doing all this work is that you want to understand how your business actually works and grows, not rely on superstitious beliefs about how your business works and grows.
  - Treat this as a [[Theory of Change|theory of change]]: action -> output -> outcome -> impact, and test assumptions at each step.
...
- When [[Data Quality|source data]] cannot support a coherent view of operations, [say the requested analysis is infeasible](https://ludic.mataroa.blog/blog/most-data-work-seems-fundamentally-worthless/) and fix the foundation or stop. A polished artifact over unusable data only hides the problem.
