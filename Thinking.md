...
- [Make predictions falsifiable, time-bounded, operationally precise, and probabilistic](https://danluu.com/zitron/).
- Prefer informative, decision-relevant predictions over tautologies, and evaluate the complete record without cherry-picking. Use calibration measures such as a [Brier score](https://en.wikipedia.org/wiki/Brier_score).
- Assess the prediction and its [[Theory of Change|causal explanation]] separately. An outcome can be right even when the reasoning is wrong!
